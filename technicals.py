...
def compute_realized_vol(close: pd.Series, window: int = 20) -> pd.Series:
    log_returns = (close / close.shift(1)).apply(lambda x: math.log(x) if pd.notna(x) and x > 0 else pd.NA)
    rv = log_returns.rolling(window).std() * math.sqrt(252)
    return rv
...
def choose_support_zone(support_20d: float | None, support_50d: float | None) -> float | None:
    ...
    if not candidates:
        return None
    # Use the nearest meaningful lower support (the higher of the valid lows)
...
def score_technical_context(tech: dict) -> dict:
    ...
    # Relative value: IV vs realized vol
